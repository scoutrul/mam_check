<template>
    <v-layout column class="testItem">
        <v-layout class="testItem__header">
            <v-flex class="testItem__icon" v-html="icon"> </v-flex>
            <v-layout class="testItem__header_2">
                <v-flex class="testItem__name">{{ name }}</v-flex>
                <v-flex class="testItem__info">
                    <template v-if="completedNum > 0">
...
export default {
    props: {
        ...
        icon: {
            type: String,
            default: '<img svg-inline src="@/assets/images/test_icon.svg" alt="icon"/>',
        },
        color: String,
